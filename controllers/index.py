...
class IndexController:
    ...
    def randomFoodLocation(self): # Cherry placement
        self.foodPoint = random.randint(100, 1000), random.randint(100, 600)

    def moveMonster(self, imgMain, img, wImg, hImg, monsterPoint, i, distance): # Monster movements
    ...
    def mouthMovement(self, imgMain, distance, i): # Mouth movement of the pacman
        if distance > 0:
    ...
    def calcDistance(self, i, MonsterPoints=False): # Distance between Monster and Pacman or Distance from past location
        if MonsterPoints:
    ...
    def update(self, imgMain):
        self.points.append([self.currentHead[0], self.currentHead[1]])
        # Check if Pacman ate the Food
        if self.foodPoint[0] - self.wFood // 2 < self.currentHead[0] < self.foodPoint[0] + self.wFood // 2 and self.foodPoint[1] - self.hFood // 2 < self.currentHead[1] < self.foodPoint[1] + self.hFood // 2:
            self.randomFoodLocation()
            self.score += 1
        # Check if Monster ate the Pacman
        if self.redPoint[0] - self.wRed // 2 < self.currentHead[0] < self.redPoint[0] + self.wRed // 2 and self.redPoint[1] - self.hRed // 2 < self.currentHead[1] < self.redPoint[1] + self.hRed // 2 or self.yellowPoint[0] - self.wYellow // 2 < self.currentHead[0] < self.yellowPoint[0] + self.wYellow // 2 and self.yellowPoint[1] - self.hYellow // 2 < self.currentHead[1] < self.yellowPoint[1] + self.hYellow // 2 or self.bluePoint[0] - self.wBlue // 2 < self.currentHead[0] < self.bluePoint[0] + self.wBlue // 2 and self.bluePoint[1] - self.hBlue // 2 < self.currentHead[1] < self.bluePoint[1] + self.hBlue // 2:
        ...
        if self.points:
            for i, _ in enumerate(self.points):
                DistanceFromPast = self.calcDistance(i)
            ...
            # Draw pacman
            self.mouthMovement(imgMain, DistanceFromPast, i)
            # Draw monsters
